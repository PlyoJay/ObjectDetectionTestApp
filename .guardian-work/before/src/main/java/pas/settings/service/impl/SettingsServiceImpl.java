...
public class SettingsServiceImpl implements SettingsSerivce {
	...
	@Override
	public EgovMap selectGuardianNotification(HashMap<String, Object> requestMap) throws EgovBizException, SQLException, Exception {
		HashMap<String, Object> params = verifiedGuardianProtegeParams(requestMap);
		...
		setting.put("protegeId", params.get("protegeId"));
		return setting;
	}
	...
	public void saveGuardianNotification(HashMap<String, Object> requestMap) throws EgovBizException, SQLException, Exception {
		...
		settingsMapper.upsertGuardianNotification(params);
	}
	...
	private HashMap<String, Object> verifiedGuardianProtegeParams(Map<String, Object> requestMap) throws EgovBizException, SQLException, Exception {
		HashMap<String, Object> params = authenticatedGuardianParams();
		Object protegeId = requestMap == null ? null : requestMap.get("protegeId");
		if (protegeId == null || String.valueOf(protegeId).trim().isEmpty()) {
			throw new EgovBizException("설정 대상 피보호자를 선택해 주세요.");
		}
		params.put("protegeId", String.valueOf(protegeId).trim());
		if (settingsMapper.countApprovedGuardianProtege(params) < 1) {
			throw new EgovBizException("연결된 피보호자만 설정할 수 있습니다.");
		}
		return params;
	}
