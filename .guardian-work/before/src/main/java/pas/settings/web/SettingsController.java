...
public class SettingsController {
	...
	@ResponseBody
	public ResponseEntity<DataSourceResult> selectGuardianNotification(@RequestBody DataSourceRequest request) throws Exception {
		DataSourceResult result = new DataSourceResult();
		LoginVO loginInfo = CmmUserDetailsHelper.getAuthenticatedUser();
		if (!isGuardian(loginInfo)) {
			result.setErrorCode(-1);
		...
		try {
			result.setData(settingsSerivce.selectGuardianNotification(request.getData()));
